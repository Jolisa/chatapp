...
public class DiscoveryConnector {
	 ...
	 */
	public DiscoveryConnector(IRMIUtils rmiUtils) throws RemoteException {
		forceWatchOnly = true;
		init(rmiUtils);
	}
	...
	public void connectToDiscoveryServer(String category, boolean watchOnly, Consumer<Iterable<IEndPointData>> updateFn) throws RemoteException {
		try {
			...
			if( watchOnly || forceWatchOnly) {
				if(!watchOnly) {
					System.err.println("[DiscoveryConnector.connectToDiscoveryServer()] DiscoveryConnector is configured for 'Watch Only', so the watchOnly parameter is being ignored.");
				}
